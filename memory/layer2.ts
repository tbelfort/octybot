...
Search for the entities to get their IDs, then store each piece of information using store_memory. Call "done" when finished.`;
  ...
  // L2 sometimes stores entities then stops without storing the facts/events
  if (filteredItems && filteredItems.length > 0) {
    ...
    // Find items that weren't stored (fuzzy match on first 60 chars)
    ...
    if (missed.length > 0) {
      await forceStore(prompt, layer1Result, turns, missed);
    }
  } else {
    // Legacy path: no filtered items, just check if anything was stored
...
export function writeConversationState(turns: ConversationTurn[], sessionId?: string): void {
...
export async function followUpPipeline(
  ...
  // Format conversation history for the LLM — include context summaries so it can
  // resolve pronouns against what was FOUND, not just what was asked.
