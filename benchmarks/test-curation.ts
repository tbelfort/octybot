...
import { callWorkersAI } from "../src/memory/workers-ai";
import { createAgent, type ClaudeModel, type ClaudeEffort, type ClaudeAgentResult, type ClaudeModelUsage } from "../src/memory/claude-agent";
import { LAYER1_MODEL, LAYER2_MODEL, VOYAGE_MODEL, DB_PATH } from "../src/memory/config";
import { getUsage, resetUsage, calculateCosts } from "../src/memory/usage-tracker";
import { scoreContext, pct, pctNum } from "./bench-utils";
...
async function main() {
  ...
  async function runQuery(q: typeof queries[0]): Promise<QueryResult> {
    ...
    const retrieveResult = await retrieveLoop(db, q.prompt, l1);
