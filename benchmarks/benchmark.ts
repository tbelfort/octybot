...
import { DB_PATH, LAYER1_MODEL, LAYER2_MODEL, VOYAGE_MODEL } from "../src/memory/config";
...
import { saveRun, closeResultsDb } from "../src/memory/results-db";
import type { QueryRecord } from "../src/memory/results-db";

import { normalize, runBatch } from "./bench-utils";
...
async function seedDb() {
  const { createNode: _createNode, createEdge: _createEdge } = await import("../memory/db-crud");
  const { storeEmbedding: _storeEmbedding } = await import("../memory/vectors");
  const { embed } = await import("../memory/voyage");

  const _db = getDb(); // init schema
  const createNode = (node: Parameters<typeof _createNode>[1]) => _createNode(_db, node);
  const createEdge = (edge: Parameters<typeof _createEdge>[1]) => _createEdge(_db, edge);
  const storeEmbedding = (nodeId: string, nodeType: string, vec: number[]) => _storeEmbedding(_db, nodeId, nodeType, vec);
...
const retrievalResults = await runBatch(RETRIEVAL_QUERIES, BATCH_SIZE, async (q) => {
  try {
...
const storeRetrieveResults = await runBatch(STORE_THEN_RETRIEVE, BATCH_SIZE, async (q) => {
  try {
