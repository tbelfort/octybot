 ...
 *
 * For tests: instantiate UsageTracker directly to get isolated state.
 */
...
export class UsageTracker {
  private usage: TokenUsage = { ...INITIAL_USAGE };

  trackTokens(layer: "l1" | "l2" | "curate" | "reconcile", input: number, output: number) {
    if (layer === "l1") {
      this.usage.l1_input += input;
      this.usage.l1_output += output;
    } else if (layer === "curate") {
      this.usage.curate_input += input;
      this.usage.curate_output += output;
    } else if (layer === "reconcile") {
      this.usage.reconcile_input += input;
      this.usage.reconcile_output += output;
    } else {
      this.usage.l2_input += input;
      this.usage.l2_output += output;
    }
  }

  trackEmbeddingTokens(tokens: number) {
...
// ── Default singleton — all existing callers keep working ──

const _default = new UsageTracker();

export function trackTokens(layer: "l1" | "l2" | "curate" | "reconcile", input: number, output: number) {
  _default.trackTokens(layer, input, output);
}

export function trackEmbeddingTokens(tokens: number) {
  _default.trackEmbeddingTokens(tokens);
}

export function getUsage(): TokenUsage {
  return _default.getUsage();
}
