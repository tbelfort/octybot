...
interface GeneratedItem {
  ...
  // Entity names mentioned in content (for creating edges from instructions/processes)
...
function generateInstruction(idx: number): GeneratedItem {
  ...
  return {
    ...
    scope: 0.1 + rng() * 0.3, // entity-specific range: 0.1–0.4
    mentionedEntities: [client, writer, tool].filter(Boolean),
  };
}
...
function generateProcess(idx: number): GeneratedItem {
  ...
  return {
    ...
    scope: 0.2 + rng() * 0.3, // tool/team range: 0.2–0.5
    mentionedEntities: [client, writer, tool].filter(Boolean),
  };
}
...
async function seedOriginalGraphDb(db: Database) {
  ...
  function createNode(node: { node_type: string; subtype?: string; content: string; salience: number; confidence: number; source: string; attributes: Record<string, any>; scope?: number }): string {
    const id = crypto.randomUUID();
    const canSummarize = (node.node_type === "instruction") ? 0 : 1;
    const scope = node.node_type === "instruction" ? (node.scope ?? 0.5) : null;
    db.prepare(
      `INSERT INTO nodes (id, node_type, subtype, content, salience, confidence, source, attributes, can_summarize, scope) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, node.node_type, node.subtype ?? null, node.content, node.salience, node.confidence, node.source, JSON.stringify(node.attributes), canSummarize, scope);
    return id;
  }
...
const graphInsertNode = graphDb.prepare(
  `INSERT INTO nodes (id, node_type, subtype, content, salience, confidence, source, attributes, can_summarize, scope) VALUES (?, ?, ?, ?, ?, ?, 'user', '{}', ?, ?)`
);
...
graphInsertNode.run(wobsBulkId, "entity", "org", "WOBS (Wolf of Blog Street) — bulk alias for edge references", 0.1, 1.0, 1, null);
...
for (const item of bulkItems) {
  ...
  const categoryToNodeType: Record<string, string> = {
    entity: "entity", fact: "fact", event: "event", opinion: "opinion",
    instruction: "instruction", process: "instruction",
  };
  const actualNodeType = categoryToNodeType[item.category] || "fact";

  const canSummarize = actualNodeType === "instruction" ? 0 : 1;
  const scope = actualNodeType === "instruction" ? (item.scope ?? 0.5) : null;
  graphInsertNode.run(nodeId, actualNodeType, item.subtype, item.content, item.salience, 1.0, canSummarize, scope);
...
// Create edges from bulk instructions/processes to their mentioned entities
for (let i = 0; i < bulkItems.length; i++) {
  const item = bulkItems[i];
  if (item.mentionedEntities && item.mentionedEntities.length > 0) {
    for (const entityName of item.mentionedEntities) {
      const targetId = entityNameToId.get(entityName);
      if (targetId) {
        graphInsertEdge.run(crypto.randomUUID(), bulkNodeIds[i], targetId, "about");
        edgeCount++;
      }
    }
  }
...
for (let i = 0; i < allBulkTexts.length; i += EMBED_BATCH_SIZE) {
  ...
  for (let j = 0; j < vectors.length; j++) {
    ...
    const nodeType = catMap[bulkItems[idx].category] || "fact";
